/**
 * =============================================================================
 * modelTrainingWorker.js — Worker completo: contexto, treino e recomendação
 * =============================================================================
 *
 * Executa em thread separada (Web Worker). Conceitos de IA:
 * - Normalização e one-hot encoding para features.
 * - Rede neural: entrada (user+product concat) → 128 → 64 → 32 → 1 (sigmoid).
 * - Loss: binaryCrossentropy (classificação binária "comprou ou não").
 * - Predição: para cada produto, score de 0–1; ordenação = ranking de recomendação.
 *
 * Em C#: equivalente a um serviço em background que recebe comandos (train/recommend)
 * e notifica a UI via eventos; os tensores seriam arrays/ML.NET em .NET.
 */
...
// Pesos das features na recomendação (soma = 1). Categoria tem mais peso,
// idade menos — reflete importância no negócio para o modelo.
const WEIGHTS = {
    category: 0.4,
    color: 0.3,
    price: 0.2,
...
// Normalização [0,1]: (val - min) / (max - min). Evita que preço ou idade
// dominem o treino por terem escalas maiores que one-hots.
...
// One-hot com peso: vetor de tamanho length com 1 na posição index, resto 0;
// depois multiplicado por weight. Em C#/ML: categorical feature encoding.
const oneHotWeighted = (index, length, weight) =>
    tf.oneHot(index, length).cast('float32').mul(weight);

/**
 * Codifica um produto em vetor numérico: [preço_norm*peso, idade_media_norm*peso,
 * one-hot categoria, one-hot cor]. Esse vetor é usado no treino e na predição.
 */
...
/**
 * Codifica o usuário: se tem compras, faz a média dos vetores dos produtos
 * comprados (perfil de consumo). Se não tem compras, vetor com idade
 * normalizada e zeros em categoria/cor — modelo usa só idade para sugerir.
 * Em C#: seria construir float[] com a mesma lógica de normalização/one-hot.
 */
...
/**
 * Monta dados de treino: para cada usuário com compras e cada produto,
 * entrada = [vetor_usuario | vetor_produto], label = 1 se comprou, 0 se não.
 * O modelo aprende P(comprou | user, product). dataSync() copia tensor para
 * array JS (equivalente a .ToArray() em C# para sair do dispositivo GPU).
 */
function createTrainingData(context) {
    ...
    return {
        ...
        inputDimention: context.dimentions * 2, // user dims + product dims
    };
}
